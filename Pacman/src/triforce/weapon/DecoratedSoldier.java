...
abstract class DecoratedSoldier implements Soldier, ArmedSoldier{
	...
	public int parry(int damages) throws BrokenItemException, DeadSoldierException {
		...
		if (weaponHealth == 0){
			...
			reducedDamages -= item.getDamageReduction();
			try{
				reducedDamages = soldier.parry((reducedDamages > 0 ? reducedDamages : 0));
			}catch(BrokenItemException e){
				soldier = e.nextItem();
				reducedDamages = e.getStrength();
				reducedDamages = parry((reducedDamages > 0 ? reducedDamages : 0));
			}
		}
